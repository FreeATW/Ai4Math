...
theorem has_only_sylow_group_not_simple {p: ℕ} [p_is_p: Fact (Nat.Prime p)]  (single: card (Sylow p G) = 1) (is_prime_factor: p ∣ card G) (non_p_group: ∀n: ℕ, card G ≠ p^n): ¬ IsSimpleGroup G := by
  ...
  ·
    ...
    have : card G = p^(card G).factorization p := by
      ...
      have : card (⊤: Subgroup G) = @card (⊤: Subgroup G) Set.fintypeUniv := card_instance_eq
      rw [this, Subgroup.card_top] at sylow_group_card
...
def get_possible_sylow_groups_card (p: ℕ) [is_prime: Fact (Nat.Prime p)] (q: ℕ) (factor: card G = p * q) (coprime: Coprime p q): possible_sylow_groups_card p q factor coprime:= by
  ...
  have lemma_p : card (Sylow p G) ∣ card G := by
    ...
    -- repeat rw [<-card_eq_fintype_card]
    let t := (Subgroup.card_quotient_dvd_card (any_sylow_group).normalizer)
    rw [card_eq_nat_card]
    rw [card_eq_nat_card] at t
    exact t
...
noncomputable def image_f {α β: Type*} [Fintype α] [DecidableEq β] (f: α -> β)  : Finset β := Finset.image f Finset.univ
...
lemma order_of_ele_in_p_group_is_pow_of_p {p: ℕ} {n : ℕ} [is_prime: Fact (Nat.Prime p)] (is_p_group: card G = p^n) : ∀x : G, ∃k < n + 1, orderOf x = p ^ k := by
  intro x
